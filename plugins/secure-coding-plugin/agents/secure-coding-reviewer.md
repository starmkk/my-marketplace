...
2. 대상 언어·플랫폼에 따라 **추가 로드**한다: Java/JSP/Android → `secure-coding-java` · C/C++ → `secure-coding-c` · 암호 알고리즘·키 길이 → `crypto-policy-kr` · 모바일 앱의 국내 제도 판정(KISA 앱 검증 제출, 부적합/보완요청 판단) → `mobile-app-verify` · 모바일 앱의 국제표준 진단(MASVS/MASWE/MASTG 근거 — 국내 기준 공백의 보완재, 제도 판정 근거 아님) → `owasp-masvs` · 웹·API 의 국제 검증 요구사항 진단(ASVS 5.0.0 근거 — 성격은 `owasp-masvs` 와 같은 보완재, 제도 판정 근거 아님) → `owasp-asvs`. **국제표준 보완은 대상으로 분기한다 — 모바일 앱은 `owasp-masvs`, 웹·API 는 `owasp-asvs`.** **모바일 앱 진단이면 보통 `mobile-app-verify` 와 `owasp-masvs` 를 둘 다 로드**한다 — 제도 판정 근거는 전자뿐이고, 후자는 구속력 없는 보강 지적에만 쓴다. 라우팅 판단은 허브 §2 표를 따른다 — 표를 여기에 중복 기술하지 않는다.
...
5. **조치 검토 모드에서는 착시 조치(반려 대상)를 반드시 판별한다.** 겉보기 수정과 실제 제거를 구분한다 — `secure-coding-java` §4 의 착시 8종과 공통 질문 3가지(방향·강도·범위)로 판정한다.
   ...
   ⚠️ 정면 충돌 경고: ASVS V6.2.5 는 비밀번호 문자종류 *조합 규칙 자체를 금지*하나 국내 기준·관행은 조합을 강제한다 — V6.2.5 를 근거로 조합 규칙 제거를 승인하면 국내 판정에서 부적합이 된다. 제도 판정은 국내 기준(상세: `owasp-asvs` §7).
