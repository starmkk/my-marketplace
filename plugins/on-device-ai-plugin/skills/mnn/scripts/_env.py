...
REQUIRED_VARS: tuple[EnvVarSpec, ...] = (
    EnvVarSpec(
        name="MNN_SOURCE_PATH",
        description="MNN 소스코드 레포 로컬 클론 경로",
        must_exist=True,
        is_dir=True,
        example="/path/to/MNN",
    ),
...
def _print_setup_guide(missing: list[EnvVarSpec], invalid: list[tuple[EnvVarSpec, str, str]]) -> None:
    ...
    print("[mnn] 환경변수가 올바르게 설정되지 않아 실행을 중단합니다.", file=sys.stderr)
    print(bar, file=sys.stderr)
    ...
    print("", file=sys.stderr)
    print("  클론 방법: git clone https://github.com/alibaba/MNN", file=sys.stderr)
    print("", file=sys.stderr)
...
def ensure_mnn_env(extra: tuple[EnvVarSpec, ...] = ()) -> dict[str, str]:
    """
    필수 환경변수 검증 게이트.
