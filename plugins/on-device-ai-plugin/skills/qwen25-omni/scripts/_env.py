"""
qwen25-omni 환경변수 검증 게이트.

본 모듈을 import한 entrypoint 스크립트는 main() 진입 직후
ensure_qwen25_omni_env()를 호출해야 한다.
...
REQUIRED_VARS: tuple[EnvVarSpec, ...] = (
    EnvVarSpec(
        name="QWEN25_OMNI_MODEL_PATH",
        description="Qwen2.5-Omni 로컬 모델 디렉토리 절대경로",
        must_exist=True,
        is_dir=True,
        example="/absolute/path/to/Qwen2.5-Omni-7B",
    ),
)
...
def _print_setup_guide(missing: list[EnvVarSpec], invalid: list[tuple[EnvVarSpec, str, str]]) -> None:
    """누락/잘못된 환경변수에 대해 설정 가이드 출력."""
    rc_path, shell_name = _detect_login_rc()
    bar = "=" * 70

    print("", file=sys.stderr)
    print(bar, file=sys.stderr)
    print("[qwen25-omni] 환경변수가 올바르게 설정되지 않아 실행을 중단합니다.", file=sys.stderr)
    print(bar, file=sys.stderr)
    ...
    print("", file=sys.stderr)
    print("  모델 다운로드:", file=sys.stderr)
    print("  huggingface-cli download Qwen/Qwen2.5-Omni-7B --local-dir /absolute/path/to/Qwen2.5-Omni-7B", file=sys.stderr)
    print("", file=sys.stderr)
...
def ensure_qwen25_omni_env(extra: tuple[EnvVarSpec, ...] = ()) -> dict[str, str]:
    """
    필수 환경변수 검증 게이트.
