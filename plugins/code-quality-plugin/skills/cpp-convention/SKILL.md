...
description: >
  ...
  "동시성", "std::thread", "std::async", "NDK", "C++17", "include 순서" 가 언급될 때 사용하라.
  들여쓰기·네이밍·브레이스 같은 기본 규약은 전역 CLAUDE.md 에 있으며 이 스킬은 그 상세판이다.
---

# C++ 코딩 컨벤션 — 상세

전역 `~/.claude/CLAUDE.md` 의 C++ 섹션이 기본 규약(들여쓰기·네이밍·브레이스·언어별 함수명)을
정의함. 이 문서는 그 상세판이며 **동시성·빌드 전제·구조 규약**을 다룸.
