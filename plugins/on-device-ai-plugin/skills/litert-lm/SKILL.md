...
소스코드가 필요한 작업(예: LiteRT-LM 소스 탐색, 빌드, API 확인 등)이 생기면
**반드시 먼저 사용자에게 확인**한다:

```
[소스 사용 흐름]
Step 1. 사용자에게 묻기:
  "로컬에 이미 LiteRT-LM 소스가 있으신가요? 있다면 경로를 알려주세요."
...
| 폴더 패턴 | `~/.claude/repo/LiteRT-LM@<version>` |

### 다운로드 방법

```bash
# 최신 버전 tag 조회
LATEST=$(git ls-remote --tags --sort=-version:refname https://github.com/google-ai-edge/LiteRT-LM \
  | grep -v '{}' | head -1 | awk '{print $2}' | sed 's|refs/tags/||')

# 최신 버전 clone (버전 미명시 시)
git clone --branch "$LATEST" --depth 1 \
  https://github.com/google-ai-edge/LiteRT-LM \
  ~/.claude/repo/LiteRT-LM@"$LATEST"

# 특정 버전 clone (예: v0.10.0)
git clone --branch v0.10.0 --depth 1 \
  https://github.com/google-ai-edge/LiteRT-LM \
  ~/.claude/repo/LiteRT-LM@v0.10.0
```

이미 `~/.claude/repo/LiteRT-LM@<version>`이 존재하면 재다운로드 없이 재사용한다.
다운로드 후 사용자에게 경로 안내.
