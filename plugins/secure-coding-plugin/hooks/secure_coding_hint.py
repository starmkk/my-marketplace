...
③ **C/C++ 는 검사 대상이 아님** — 화이트리스트 미포함이고 패턴도 없다. 누락이
   아니라 "공백 영역만" 원칙의 귀결이다(2026-09-18 기준 경계 판정, 정본 대조):
   자체 암호 알고리즘은 기준21(CWE-327, p.330 "자체 알고리즘 개발 금지"),
   `rand`/`srand` 키 생성은 기준25(CWE-330, p.364 가 C `rand()` 를 직접 지명),
   JNI 자원 누수·Null 역참조·버퍼 복사는 기준40(p.445)·39(p.433)·16(p.303)이
   각각 덮는다 — 전부 국내 대응이라 훅이 아니라 `secure-coding-c` 소관이다.
   국내 3개 체계가 모두 침묵하는 유일한 네이티브 영역은 MASVS-RESILIENCE-4
   (MASWE-0064·0065 디버거·동적분석 탐지 미구현)인데, 하필 전부 **부재형**이라
   ①에 걸려 정규식으로 잡을 수 없다. 즉 C/C++ 를 열어도 넣을 패턴이 없다.
   ⚠️ 확장자만 추가하는 변경은 하지 말 것 — 매칭될 패턴이 없어 무의미하다.
   NDK C/C++ 진단이 필요하면 훅이 아니라 `secure-coding-reviewer` 에이전트를
   쓴다(그쪽은 `secure-coding-c` 를 로드해 C/C++ 를 정식 지원한다).
...
# ⚠️ C/C++(.c·.cpp·.h 등)는 사정이 다르다 — 한 줄 추가로 끝나지 않는다.
#    넣어도 매칭될 패턴이 없고, 넣을 만한 패턴도 없다(위 커버리지 한계 ③).
