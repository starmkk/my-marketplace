...
| `secure-coding-plugin` | 1.0.7 | 스킬 7종 + 에이전트 1종 + 훅 1종 | 한국 전자정부 SW 개발보안(시큐어코딩) 공식 가이드 레퍼런스 + OWASP MASVS·ASVS 국제표준 보완 |
...
보안약점 진단, 정적분석 결과의 오탐/정탐 판정, 보완조치가 착시 조치(반려 대상)인지 확인이 필요할 때 호출합니다. 허브 `secure-coding-kr` 의 라우터 표에 따라 언어·맥락별 판정 기준 스킬을 로드해 쓰며, 코드를 대신 수정하지 않고 `SendMessage` 로 보고서 전문을 전달합니다. **대상 언어는 Java/JSP/Android 와 C/C++ 양쪽입니다** — C/C++ 는 `secure-coding-c` 를 로드해 메모리 버퍼 오버플로우·포맷 스트링·해제 후 사용·취약한 API 를 다루며, NDK 네이티브 모듈도 범위입니다. 다만 보고서 기재값(기준번호·공식 약점명·CWE)의 정본은 49개 기준이고, **C 가이드 58개 체계의 CWE 는 보고서 기재용이 아닙니다** — 두 체계는 번호·명칭·CWE 가 모두 별개입니다. 오탐 판정이 판단 결정적 작업이므로 실행 모델은 `model: opus` 로 고정되어 있습니다(v1.0.2).
...
- 경고는 **두 채널로 동시에** 나갑니다. `systemMessage` 는 사용자 화면에만 표시되고, `hookSpecificOutput.additionalContext` 는 Claude 컨텍스트에만 주입됩니다 — 둘은 대체재가 아니라 수신자가 다른 별개 채널이라, 한쪽만 내면 상대에게 닿지 않습니다. 스킬 로드를 유도하려면 `additionalContext` 가 필요합니다.
- 모델용 문구는 **사실 서술**로 씁니다 — 명령형은 프롬프트 인젝션 방어에 걸려 컨텍스트로 쓰이지 않고 사용자에게 노출됩니다.
- 메시지는 두 채널 모두 항상 5줄 이내입니다 — 동시 매칭이 3건을 넘으면 초과분을 건수로 접습니다.
- 커버리지 한계가 셋 있어 **훅이 조용하다고 점검이 끝난 것이 아닙니다**: ① 정규식은 존재하는 문자열만 잡으므로 `FLAG_SECURE` 미설정처럼 **부재가 취약점인 항목**은 구조적으로 탐지할 수 없습니다. ② `Bash` 명령이나 외부 프로세스가 파일을 고치면 `Edit`·`Write` matcher 의 `PostToolUse` 훅은 발동하지 않습니다. ③ **C/C++ 는 검사 대상이 아닙니다** — 아래 참조.
- **C/C++ 가 훅 대상이 아닌 이유** (2026-09-18 기준 경계 판정): 누락이 아니라 "공백 영역만" 원칙의 귀결입니다. 자체 암호 알고리즘은 **기준21**(CWE-327, p.330), `rand`/`srand` 키 생성은 **기준25**(CWE-330, p.364 — C `rand()` 를 직접 지명), JNI 자원 누수·Null 역참조·버퍼 복사는 **기준40**(p.445)·**39**(p.433)·**16**(p.303)이 각각 덮는 **국내 대응 영역**이라 `secure-coding-c` 소관입니다. 국내 3개 체계가 모두 침묵하는 유일한 네이티브 영역은 MASVS-RESILIENCE-4(MASWE-0064·0065 디버거·동적분석 탐지 **미구현**)인데 전부 **부재형**이라 한계 ①에 걸려 정규식으로 잡히지 않습니다 — 확장자를 열어도 넣을 패턴이 없습니다. **NDK C/C++ 진단은 훅이 아니라 `secure-coding-reviewer` 에이전트**를 쓰십시오(`secure-coding-c` 를 로드해 정식 지원합니다).
